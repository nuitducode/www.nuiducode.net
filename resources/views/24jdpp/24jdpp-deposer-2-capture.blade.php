...
<body>
    ...
    <?php
    $appFile = storage_path('app').'/public/vingtquatre-apps/jour-'.session('depot_24_app_jour').'/'.Crypt::decryptString(session('depot_24_app_jeton')).'/app.py';
    ...
    file_put_contents(dirname($appFile).'/app-screenshot.py', $updatedContent);
    //$escapedScript = htmlspecialchars($updatedContent, ENT_QUOTES);
    ?>

    <!-- Lancer Pyxel avec le script modifié -->
    <pyxel-run root="{{asset('storage/vingtquatre-apps/jour-'.session('depot_24_app_jour').'/'.Crypt::decryptString(session('depot_24_app_jeton')))}}" name="app-screenshot.py"></pyxel-run>
